...
import { RouteUseLocationModel, RouteUseParamsModel } from './models/inedx';
...
function ModalSwitch() {
  let location: RouteUseLocationModel = useLocation();
...
function Thumbnail({ color }: propsModel) {
  return (
    <div
      style={{
        width: 50,
...
function Image({ color }: propsModel) {
  return (
    <div
      style={{
        width: '100%',
...
function ImageView() {
  let { id }: RouteUseParamsModel = useParams();
  let image = IMAGES[parseInt(id as string, 10)];
